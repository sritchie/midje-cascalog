...
(defmacro defmirror
  {:private true}
  [alias & more]
  (let [midje-sym (symbol "midje.sweet" (str alias))
    ...
    `(do (defmacro ~sym-a
         ...
         (defmacro ~sym-b
           ...
           (build-fact?<- args# '~(symbol (str *ns*)
                                          (str sym-a))))
         ~@(when more [(list* `defmirror more)]))))

(defmirror fact)
(defmirror facts)
(defmirror future-fact)
(defmirror future-facts)
(defmirror pending-fact)
(defmirror pending-facts)
(defmirror incipient-fact)
